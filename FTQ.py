...
st.set_page_config(page_title="KOSTAL - HYUNDAI/KIA", layout="wide")

COLOR_KOSTAL = "#004b87"
COLOR_TARGET = "#28a745"
COLOR_ACTION = "#dc3545"

st.title("📊 FTQ - HYUNDAI/KIA")
st.markdown("*(Working model 1 Shift x 5 days)*")

@st.cache_data(ttl=300)
def procesar_datos_kia(filepath):
...
if os.path.exists(archivo_bd):
    df_prod, df_ftq, df_def = procesar_datos_kia(archivo_bd)
    ...
    if len(versiones_disponibles) > 0:
        ...
        titulo_seccion = f"Versión {version_sel}"
        ...
        st.warning("Aún no hay datos cargados en la base.")
else:
